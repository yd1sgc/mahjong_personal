...
def main():
    db_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'local_mahjong_v2.db')
    print(f"ローカルDBに接続しています: {db_path}")
    ...
    if not updates:
        print("修正が必要なデータは見つかりませんでした。すべて正しい状態です。")
        conn.close()
        return
    ...
    # 修正を一括適用
    c.executemany("UPDATE rounds SET round_index = ? WHERE round_id = ?", updates)
    conn.commit()
    conn.close()
    
    print("正常にローカルデータの修正が完了しました！")
